...
*/
template< class type >
ID_INLINE void idListSwap( type &a, type &b ) {
	type c = a;
	a = b;
	b = c;
}
...
ID_INLINE void idList<_type_,_tag_>::Swap( idList &other ) {
	idListSwap( num, other.num );
	idListSwap( size, other.size );
	idListSwap( granularity, other.granularity );
	idListSwap( list, other.list );
}
