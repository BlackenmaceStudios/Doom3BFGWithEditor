...
static void SaveWindow( void ) {
	oldWidth = renderSystem->GetWidth();
	oldHeight = renderSystem->GetHeight();
}
...
static void OutlineNormalMap( byte *data, int width, int height, int emptyR, int emptyG, int emptyB ) {
	...
	orig = (byte *)Mem_Alloc( width * height * 4, TAG_TOOLS );
...
static triHash_t *CreateTriHash( const srfTriangles_t *highMesh ) {
	...
	hash = (triHash_t *)Mem_Alloc( sizeof( *hash ), TAG_TOOLS );
	...
	hash->linkBlocks[hash->numLinkBlocks] = (triLink_t *)Mem_Alloc( MAX_LINKS_PER_BLOCK * sizeof( triLink_t ), TAG_TOOLS );
	hash->numLinkBlocks++;
...
// jmarshall end
	...
	for ( j = 0 ; j < 3 ; j++ ) {
		sampledNormal += bary[j] * highMesh->verts[ highMesh->indexes[ faceNum * 3 + j ] ].GetNormal();
	}
...
#define SKIP_MIRRORS
			...
			for ( k = 0 ; k < 3 ; k++ ) {
				...
				normal += bary[k] * lowMesh->verts[ index ].GetNormal();
				tangents[0] += bary[k] * lowMesh->verts[ index ].GetTangent();
				tangents[1] += bary[k] * lowMesh->verts[ index ].GetBiTangent();
			}
...
static void RenderBumpTriangles( srfTriangles_t *lowMesh, renderBump_t *rb ) {
	...
	// to the true surface.
	idVec3	*lowMeshNormals = (idVec3 *)Mem_ClearedAlloc( lowMesh->numVerts * sizeof( *lowMeshNormals ), TAG_TOOLS );
	...
	for ( j = 0 ; j < lowMesh->numIndexes ; j+=3 ) {
		...
		qglClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );
		qglRasterPos2f( 0, 1 );
		qglPixelZoom( renderSystem->GetWidth() / (float)rb->width, renderSystem->GetHeight() / (float)rb->height );
...
static void InitRenderBump( renderBump_t *rb ) {
	...
	// local normal map
	rb->localPic = (byte *)Mem_Alloc( c, TAG_TOOLS );

	// global (object space, not surface space) normal map
	rb->globalPic = (byte *)Mem_Alloc( c, TAG_TOOLS );

	// color pic for artist reference
	rb->colorPic = (byte *)Mem_Alloc( c, TAG_TOOLS );

	// edgeDistance for marking outside-the-triangle traces
	rb->edgeDistances = (float *)Mem_Alloc( c, TAG_TOOLS );
...
void RenderBumpFlat_f( const idCmdArgs &args ) {
	...
	sumBuffer = (int *)Mem_Alloc( width * height * 4 * 4, TAG_TOOLS );
	memset( sumBuffer, 0, width * height * 4 * 4 );
	buffer = (byte *)Mem_Alloc( width * height * 4, TAG_TOOLS );

	colorSumBuffer = (int *)Mem_Alloc( width * height * 4 * 4, TAG_TOOLS );
	memset( sumBuffer, 0, width * height * 4 * 4 );
