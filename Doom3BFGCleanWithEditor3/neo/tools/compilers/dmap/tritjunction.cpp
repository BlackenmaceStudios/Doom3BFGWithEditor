...
static mapTri_t *FixTriangleAgainstHashVert( const mapTri_t *a, const hashVert_t *hv ) {
	...
	for ( i = 0 ; i < 3 ; i++ ) {
		...
		idVec3 splitNormal;
		splitNormal[0] = v1->normal[0] + frac * ( v2->normal[0] - v1->normal[0] );
		splitNormal[1] = v1->normal[1] + frac * ( v2->normal[1] - v1->normal[1] );
		splitNormal[2] = v1->normal[2] + frac * ( v2->normal[2] - v1->normal[2] );
