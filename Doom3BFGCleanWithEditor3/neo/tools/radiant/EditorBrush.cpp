	...
	for ( int i = 0; i < model->NumSurfaces(); i++ ) {
		...
		for ( int j = 0; j < tri->numIndexes; j += 3 ) {
			for ( int k = 0; k < 3; k++ ) {
				...
				qglTexCoord2f( tri->verts[index].GetTexCoord().x, tri->verts[index].GetTexCoord().y );
...
face_t *Face_Alloc(void) {
	...
	face_t *f = (face_t *) Mem_ClearedAlloc(sizeof(*f), TAG_TOOLS);
	...
	for (face = b->brush_faces; face; face = face->next) {
		...
		for (i = 0; i < w->GetNumPoints(); i++) {
			...
			if ( p2.Compare( (*w)[(i + 1) % w->GetNumPoints()].ToVec3(), TINY_EPSILON ) ) {
				...
				neww->InsertPoint( idVec5(point.x, point.y, point.z, 0, 0), (i + 1) % w->GetNumPoints() );
				break;
			}
			else if ( p2.Compare( (*w)[(i - 1 + w->GetNumPoints()) % w->GetNumPoints()].ToVec3(), TINY_EPSILON ) ) {
				neww = new idWinding( *w );
				neww->InsertPoint( idVec5(point.x, point.y, point.z, 0, 0), i );
				break;
			}
		}
