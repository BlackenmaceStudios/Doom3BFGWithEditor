...
void idGLDrawableMaterial::draw(int x, int y, int w, int h) {
	...
	if (mat) {
		...
		if (worldDirty) {
			...
			tris->verts[0].SetNormal(tris->verts[1].xyz.Cross(tris->verts[3].xyz));
			tris->verts[1].SetNormal(tris->verts[0].GetNormal());
			tris->verts[2].SetNormal(tris->verts[0].GetNormal());
			tris->verts[3].SetNormal(tris->verts[0].GetNormal());
			AddTris(tris, mat);
