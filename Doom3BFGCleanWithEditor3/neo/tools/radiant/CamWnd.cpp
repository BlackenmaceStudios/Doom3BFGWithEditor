...
void CCamWnd::OnPaint() {
	...
	if (!qwglMakeCurrent(dc.m_hDC, hGLRC)) {
	...
	if ( v && *v ) {
		...
		if ( !stricmp( name, v ) ) {
			...
			if (md5) {
				...
				refent.joints = ( idJointMat * )Mem_Alloc16( refent.numJoints * sizeof( *refent.joints ), TAG_TOOLS );
...
void Tris_ToOBJ(const char *outFile, idTriList *tris, idMatList *mats) {
	...
	if ( f ) {
		...
		for (i = 0; i < tris->Num(); i++) {
			...
			for (j = 0; j < tri->numVerts; j++) {
				f->Printf( "vt %f %f\n", tri->verts[j].st[0], 1.0f - tri->verts[j].st[1] );
			}
			for (j = 0; j < tri->numVerts; j++) {
				f->Printf( "vn %f %f %f\n", tri->verts[j].normal[0], tri->verts[j].normal[1], tri->verts[j].normal[2] );
			}
