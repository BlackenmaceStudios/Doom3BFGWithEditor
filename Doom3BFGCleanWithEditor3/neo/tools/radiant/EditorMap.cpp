...
brush_t *BrushFromMapPatch(idMapPatch *mappatch, idVec3 origin) {
	...
	for (int i = 0; i < mappatch->GetWidth(); i++) {
		for (int j = 0; j < mappatch->GetHeight(); j++) {
			...
			pm->ctrl(i, j).SetTexCoordNative((*mappatch)[j * mappatch->GetWidth() + i].st[0], (*mappatch)[j * mappatch->GetWidth() + i].st[1]);
		}
	}
	...
	if ( b->pPatch ) {
		...
		for ( int i = 0; i < b->pPatch->width; i++ ) {
			for ( int j = 0; j < b->pPatch->height; j++ ) {
				...
				(*patch)[j*patch->GetWidth()+i].SetTexCoord(b->pPatch->ctrl(i, j).st[0], b->pPatch->ctrl(i, j).st[1]);
			}
		}
